...
# Country selection for norms
country = st.selectbox("Select Country for Norms", ["Australia", "USA", "Canada", "UK"])
# Override norms for r, θ, ð based on country
...
if delayed:
    ...
    # Apply styling to highlight numbers and percentages
    styled_goals = []
    for g in raw_goal_list:
        styled = re.sub(r"(\d+%?)", r"<span style='color:#1f77b4; font-weight:bold;'>\1</span>", g)
        styled_goals.append(styled)
    # Display styled goals
    st.markdown("<ul>" + "".join(f"<li>{sg}</li>" for sg in styled_goals) + "</ul>", unsafe_allow_html=True)
    # Download raw goal text
    st.download_button("Download SMART Goals (TXT)", "\n".join(raw_goal_list), file_name=f"{child_name}_goals.txt")("Download SMART Goals (TXT)", "\n".join(goal_list), file_name=f"{child_name}_goals.txt")